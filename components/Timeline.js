...
class Timeline {
	...
	async fetchCountryDetails() {
		...
		// i should be free to get back on discord if you are
		// ok, I'll just be 5 minutes or so
		// no problem, i might even go for dinner before then, trying to figure this out first though
			...
			if (typeof regionLine[1] !== 'undefined') {
				...
				if (cases > 0) {
					...
					try {
						...
						day.addData(cases, deaths, recovered, countryName, date, population, latlng, continent);
					} catch (error) {
						console.log(error);
					}
				}
